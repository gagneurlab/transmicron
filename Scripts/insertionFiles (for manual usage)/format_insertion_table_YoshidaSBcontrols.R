...
#insertions <- read.table("/s/project/transposon/Output/Snakemake/Input/SB+G418_control.tgz", fill=T)
insertions <- read.table("/s/project/transposon/Output/Snakemake/Input/SB_control_1.bed", fill=T)
#removeNAs
# insertions <- insertions[-(which(insertions=="", arr.ind = T)[,1]),]
# insertions <- insertions[-which(insertions$V4=="users", arr.ind=T),]
...
end(unselected_Granges)[which(unselected_Granges$orientation=="+")] <- end(unselected_Granges[which(unselected_Granges$orientation=="+")])+1
...
saveRDS(SleeepinBeauty_Granges_correct, "/s/project/transposon/Output/Snakemake/yoshidaControlsSB/correct_ranges.RData")
