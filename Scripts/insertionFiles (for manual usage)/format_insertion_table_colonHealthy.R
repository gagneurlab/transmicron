...
insertions<-fread("/s/project/transposon/Output/Snakemake/Input/insertion_inputs/Anja_colon/anja_download/ColonPBGene_allcohorts-ext.BED")

#subset for colon samples
colon <- which(grepl(insertions$tissue, pattern="TU"))
healthy_colon <- which(grepl(insertions$tissue, pattern="HT"))
remove <- c(colon, healthy_colon)
nonSelectiveInsertions <- insertions[-remove,]
#specify genome
...
colon_Granges<-insertion_table_to_Granges(nonSelectiveInsertions)
...
donor_chromosome ="chr10"
on_donor <- which(as.vector(seqnames(PiggyBac_Granges_correct))==donor_chromosome)
if(length(on_donor)!=0){
  PiggyBac_Granges_correct <- PiggyBac_Granges_correct[-on_donor]
  print("excluded insertions:")
  print(length(on_donor))}
names(PiggyBac_Granges_correct) <- 1:length(PiggyBac_Granges_correct)
saveRDS(PiggyBac_Granges_correct, "/s/project/transposon/Output/Snakemake/wholeBodyHealthy/correct_ranges.RData")
